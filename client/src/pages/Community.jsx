...
const Community = () => {
  ...
  const imageLikeToggle = async (id) =>{
    try{
      ...
      if(data.success){
        toast.success(data.message || 'Liked/Unliked successfully');
        await fetchCreations();
      }else{
        toast.error(data.message || 'Failed to like/unlike creation');
      }
    }catch(err){
      toast.error(err.message || 'Failed to like/unlike creation');
    }
  }
  ...
  return !loading ? (
        ...
        {creations.map((creation,index)=>{
          return(
            <div key={(index)} className='relative group inline-block pl-3 pt-3 w-full sm:max-w-1/2 lg:max-w-1/3'> 
              ...
              <div className='absolute bottom-0 top-0 right-0 left-3 flex gap-2 items-end justify-end group-hover:justify-between p-3 group-hover:bg-gradient-to-b from-transparent to-black/50 text-white rounded-lg'>
    ...
    <div className='flex justify-center items-center h-full'>
      <span className='w-10 h-10 my-1 rounded-full border-3 border-primary border-t-transparent animate-spin inline-block'></span>
